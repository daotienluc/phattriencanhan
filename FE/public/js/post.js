...
async function fetchPosts() {
    try {
        ...
        for (const post of data) {
            ...
            if (username) {
                try {
                    const likeResponse = await fetch(`http://localhost:3004/api/posts/${post.id}/like-status?username=${username}`);
        ...
        submitCommentButtons.forEach(button => {
            button.addEventListener('click', async function() {
                ...
                if (commentContent) {
                    try {
                        const response = await fetch(`http://localhost:3004/api/posts/${postId}/comments`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
